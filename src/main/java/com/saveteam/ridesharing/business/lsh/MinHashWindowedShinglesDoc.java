...
public class MinHashWindowedShinglesDoc<T> {
	...
	public MinHashWindowedShinglesDoc(Set<T> S1, Set<T> S2)
	{
...
//			System.out.println(shingle);
	...
	public static void main(String[] args) {
		...
		String s1 = "";
		try {
			s1 = new Scanner(new File("Deduplication/resource/sample1.txt")).useDelimiter("\\Z").next();//reading the complete file as string
		...
		String s2 = "";
		try {
			s2 = new Scanner(new File("Deduplication/resource/sample2.txt")).useDelimiter("\\Z").next();
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
...
//		Set setB = Create_Shingles(s2, shingle_length);
		...
		MinHashWindowedShinglesDoc<String> obj = new MinHashWindowedShinglesDoc<String>(setA, setB);
